...
class SVMLive:
	...
	def trade(self, coin):
		...
		try:
			...
			self.CreateSVM(coin, x_train, x_test, y_train, y_test, x_test_dates, apiDateTime, apiRequest)

	def CreateSVM(self, coin, x_train, x_test, y_train, y_test, x_test_dates, apiDateTime,apiRequest):
		...
		self.runTrading(coin, predictions, y_test, len(predictions), x_test_dates, rbf, apiRequest) #try and trade a profit using the predictions


	def saveToFile(self, coin, predictions, x, y, mse, rmse, mae, confidence): #save the results of the prediction to an excel file

		workbook = xlsxwriter.Workbook('results/SVM/SVMLive-' + coin + '-Predictions.xlsx')
	...
	def runTrading(self, coin, predictions, y_test, numberOfDays,x_test_dates, rbf, apiRequest):
		...
		while(datetime.today() <= (startDay + timedelta(days=2))): #run the live bot for 2 days
			...
			predictions = self.getNewPredictions(rbf, x_test, y_test,x_test_dates, startTime, finishTime, coin) #get predictions of the new data
		...
		workbook = xlsxwriter.Workbook('results/SVM/SVMLive-' + coin + '-TradingBot.xlsx')
		worksheet = workbook.add_worksheet()
		worksheet.set_column('A:A', 25)
		worksheet.set_column('B:B', 15)
		worksheet.set_column('C:C', 15)
	...
	def getNewPredictions(self, rbf, x_test, y_test, x_test_dates, startTime, finishTime, coin):
		StdS_X = StandardScaler()
		StdS_y = StandardScaler()
		y_test_normalised = np.array(y_test)
		y_test_normalised = StdS_y.fit_transform(y_test_normalised.reshape(-1,1))
