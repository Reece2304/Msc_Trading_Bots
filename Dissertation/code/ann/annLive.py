...
class ANNLive:
    ...
    def trade(self, coin):
        ...
        try:
            ...
            self.CreateANN(coin, x_train, x_test, y_train, y_test, x_test_dates, apiDateTime, apiRequest)
    ...
    def CreateANN(self, coin, x_train, x_test, y_train, y_test, x_test_dates, apiDateTime, apiRequest):
        ...
        model.add(layers.LSTM(units=128, activation='relu', input_shape=(5,1), dropout=0.1))
        ...
        figure_canvas = FigureCanvasTkAgg(figure, self.page)
        axes = figure.add_subplot()
        axes.set_title('Live Prediction Price for: ' + coin)
        axes.set_xlabel('Date')
        axes.set_ylabel('Price in USD')
        ...
        self.saveToFile(coin, predictions, x_test_dates, y_test, mse,rmse,mae) #save the results to an excel file

        self.runTrading(coin, predictions, y_test, len(predictions), x_test_dates, model, apiRequest) #try and trade a profit using the predictions


    def saveToFile(self, coin, predictions, x, y, mse, rmse, mae): #save the results of the prediction to an excel file

        workbook = xlsxwriter.Workbook('results/ANN/ANNLive-' + coin + '-Predictions.xlsx')
    ...
    def runTrading(self, coin, predictions, y_test, numberOfDays,x_test_dates, model, apiRequest):
        ...
        while(datetime.today() <= (startDay + timedelta(days=2))): #run the live bot for 2 days
            ...
            predictions = self.getNewPredictions(model, x_test, y_test,x_test_dates, startTime, finishTime, coin) #get predictions of the new data

            newEntry = self.runNewTrade(predictions, y_test, len(predictions), startTime, currentAssetAmount, Currency, holdProfit, profit, HoldAmount, startingCurrency) #try and trade a profit using the predictions
        ...
        workbook = xlsxwriter.Workbook('results/ANN/ANNLive-' + coin + '-TradingBot.xlsx')
        worksheet = workbook.add_worksheet()
        worksheet.set_column('A:A', 25)
        worksheet.set_column('B:B', 15)
        worksheet.set_column('C:C', 15)
    ...
    def getNewPredictions(self, model, x_test, y_test, x_test_dates, startTime, finishTime, coin):
        StdS_X = StandardScaler()
        StdS_y = StandardScaler()
        y_test_normalised = np.array(y_test)
        y_test_normalised = StdS_y.fit_transform(y_test_normalised.reshape(-1,1))
        x_test = StdS_X.fit_transform(x_test)
        predictions = model.predict(x_test) #get a list of predictions using the test data
        predictions = StdS_y.inverse_transform(predictions)
        self.updatePlot(y_test, x_test_dates, predictions, startTime, finishTime, coin) #redraw the plot on the screen
        return predictions

    def updatePlot(self, y_test, x_test_dates, predictions, startTime, finishTime, coin):
        if (len(self.predictions) <= 0):
            self.predictions.append(predictions[0])
            self.y_test.append(y_test[0])
            self.x_test_dates.append(x_test_dates.iloc[0,0])
        else:
            self.predictions.append(predictions[0])
            self.y_test.append(y_test[0])
            self.x_test_dates.append(x_test_dates.iloc[0,0])
        ...
        # print(self.y_test)
        # print(self.x_test_dates)
                #plot the predictions and display to the user
        Pplot = {'Date' : self.x_test_dates,
                'Actual_Price': self.y_test 
                } #create a dictionary with dates and close values

        Pplot2 = { 'Date' : self.x_test_dates,
        ...
        axes.set_xlabel('Date')
        axes.set_ylabel('Price in USD')
        axes.set_xticklabels(self.x_test_dates, fontsize=7)
    ...
    def runNewTrade(self, predictions, y_test, numberOfRuns, startTime, currentAssetAmount, Currency, holdProfit, profit, HoldAmount, startingCurrency):
