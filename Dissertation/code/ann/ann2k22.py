...
class ANN2022:
	...
	def trade(self, coin):
		...
		try:
			...
			figure = plt.Figure(figsize=(6, 6), dpi=105) #create the plot
		...
		try:
			...
			print("no file") #if there's no data then call the api request
			url = 'https://rest.coinapi.io/v1/ohlcv/COINBASE_SPOT_' + apiRequest +'_USD/history?period_id=8HRS&time_start=2021-12-31T00:00:00&time_end=2022-12-31T00:00:00&limit=1000' #get training data
			headers = {'X-CoinAPI-Key' : 'F7F21667-42EE-466D-B32D-DB4E2D15E9EE'}
			#headers = {'X-CoinAPI-Key' : '8C728603-6D0B-45CF-87CE-5D56F7D95BC8'}
			r = requests.get(url, headers=headers)
			data = r.json()
			...
			self.CreateANN(coin, x_train, x_test, y_train, y_test, x_test_dates)

	def CreateANN(self, coin, x_train, x_test, y_train, y_test, x_test_dates):
		...
		history = model.fit(x_train,y_train,epochs=400, batch_size=25,shuffle=False,verbose=2)
		...
		figure_canvas = FigureCanvasTkAgg(figure, self.page)
		axes = figure.add_subplot()
		axes.set_title('Prediction Price for 2022 for: ' + coin)
		axes.set_xlabel('Date')
		axes.set_ylabel('Price in USD')
	...
	def saveToFile(self, coin, predictions, x, y, mse, rmse, mae): #save the results of the prediction to an excel file
		...
		workbook = xlsxwriter.Workbook('results/ANN/ANN2022-' + coin + '-Predictions.xlsx')
	...
	def runTrading(self, coin, predictions, y_test, numberOfDays,x_test_dates):
		...
		for i in range(0, numberOfDays): #run the trading simulation for as many days as the predictions
			...
			self.pProfit2.config(text = str(round((profit/ startingCurrency) * 100, 2)) + "%")
			...
			if i < numberOfDays - 1:
				...
				data.append([x_test_dates.iloc[i,0], currentCoinPrice, action, Currency, currentAssetAmount, profit] )
		...
		workbook = xlsxwriter.Workbook('results/ANN/ANN2022-' + coin + '-TradingBot.xlsx')
		worksheet = workbook.add_worksheet()
		worksheet.set_column('A:A', 25)
		worksheet.set_column('B:B', 15)
		worksheet.set_column('C:C', 15)
