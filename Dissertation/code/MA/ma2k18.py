...
class MA2018:
    ...
    def ComputeMA(self,coin, data, MA5, MA10):
        ...
        for i in range(0,4):
            data.loc[i, ['5MA']] = MA5.loc[len(MA5.index) - 4 + i] #write the 5MA from the training data into the first 5 values of the testing MA

        for i in range(0,9):
            data.loc[i, ['10MA']] = MA10.loc[len(MA10.index) - 9 + i] #write the 10MA from the training data into the first 10 values of the testing MA
    ...
    def saveToFile(self, coin, data): #save the results of the prediction to an excel file
        ...
        workbook = xlsxwriter.Workbook('results/MA/MA2018-' + coin + '-Predictions.xlsx')
    ...
    def runTrading(self, coin, data, numberOfDays):
        ...
        workbook = xlsxwriter.Workbook('results/MA/MA2018-' + coin + '-TradingBot.xlsx')
        worksheet = workbook.add_worksheet()
        worksheet.set_column('A:A', 25)
        worksheet.set_column('B:B', 15)
        worksheet.set_column('C:C', 15)
