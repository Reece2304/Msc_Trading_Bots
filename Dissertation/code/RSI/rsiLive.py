...
class RSILive:
	...
	def trade(self, coin):
		...
		try:
			...
			self.ComputeRSI(coin,df_test, df_train['RSI_20'], apiDateTime,apiRequest)

	def ComputeRSI(self, coin, data, RSI_20, apiDateTime, apiRequest):
		...
		data['Buy'] = np.where(data['RSI_20'] <= 25, 1,0)
		data['Sell'] = np.where(data['RSI_20'] >= 60, 2,0)
		...
		SellLine = { 'Date' : data['time_period_start'],
					'60' : [60 for index , row in data.iterrows() ]
				 }
		...
		Plot2 = pd.DataFrame(SellLine, columns = ['Date', '60'])
		...
		figure_canvas = FigureCanvasTkAgg(figure, self.page)
		axes = figure.add_subplot()
		axes.set_title('RSI Live for: ' + coin)
		axes.set_xlabel('Date')
		...
		figure_canvas = FigureCanvasTkAgg(figure, self.page)
		axes = figure.add_subplot()
		axes.set_title('Live Price for: ' + coin)
		axes.set_xlabel('Date')
		axes.set_ylabel('Price in USD')
		axes.set_xticklabels(plot['Date'], fontsize=7)
	...
	def saveToFile(self, coin, data): #save the results of the prediction to an excel file
		workbook = xlsxwriter.Workbook('results/RSI/RSILive-' + coin + '-Calculations.xlsx')
	...
	def runTrading(self, coin, data, numberOfDays, apiRequest):
		...
		while(datetime.today() <= (startDay + timedelta(days=2))): #run the live bot for 2 days
			...
			Newdata = self.computeNewRSI(data, startTime, finishTime, coin, df_test) #get predictions of the new data
		...
		workbook = xlsxwriter.Workbook('results/RSI/RSILive-' + coin + '-TradingBot.xlsx')
		worksheet = workbook.add_worksheet()
		worksheet.set_column('A:A', 25)
		worksheet.set_column('B:B', 15)
		worksheet.set_column('C:C', 15)
	...
	def computeNewRSI(self, data, startTime, finishTime, coin, df_test):
		prices = []
		self.close_price.append(df_test['price_close'][0])
		self.dates.append(df_test['time_period_start'][0])

		if len(self.RSIs) < 20:
			for i in range(0, (20-len(self.RSIs))): #for loop that gets the last n amount of prices
				prices.append(data.loc[len(data.index) - (20-len(self.RSIs)) + i, ['price_close']][0])

			print(self.close_price)
			print("Lenght of RSI : " + str( len(self.RSIs)))
			for i in range(0, len(self.RSIs)): #gets the remaining prices that have already been added
				print("i: "+ str(i))
				print("Close Price - i:  " + str(self.close_price[len(self.RSIs) - i]))
				prices.append(self.close_price[len(self.RSIs) - i])

		else:

			for i in range(0, 20):
				prices.append(self.close_price[len(self.RSIs) - i]) #append the close prices starting from the end

		prices.append(df_test['price_close'][0]) #repace the oldest price with the newest price called from the api
		print(prices)

		price = pd.DataFrame(prices, columns = ['price_close'])
		price.ta.rsi(close= 'price_close', length = 20, append=True) #calculate the rsi of the new datapoint 

		df_test['RSI_20'] = price['RSI_20'][len(price.index)-1]

		df_test['Buy'] = np.where(df_test['RSI_20'] <= 25, 1,0)
		df_test['Sell'] = np.where(df_test['RSI_20'] >= 70, 2,0)

		print(df_test)

		self.updatePlot(df_test, df_test['RSI_20'], startTime, finishTime, coin)

		return df_test

	def updatePlot(self, data, RSI, startTime, finishTime, coin):
		self.RSIs.append(RSI[0])
		print("RSIs:")
		print(self.RSIs)

		RsiPlot = {'Date' : self.dates,
			'RSI_20' : self.RSIs
		}

		SellLine = { 'Date' : self.dates,
			'Sell Line' : [70 for i in range(0, len(self.RSIs)) ]
		}

		BuyLine = { 'Date' : self.dates,
			'Buy Line' : [25 for i in range(0, len(self.RSIs)) ]
		}


		Plot2 = pd.DataFrame(SellLine, columns = ['Date', 'Sell Line'])
		Plot3 = pd.DataFrame(BuyLine, columns = ['Date', 'Buy Line'])
		Plot = pd.DataFrame(RsiPlot, columns = ['Date', 'RSI_20'])
		Plot.set_index("Date", inplace=True)
		Plot2.set_index("Date", inplace=True)
		Plot3.set_index("Date", inplace=True)
		figure = plt.Figure(figsize=(5, 5), dpi=95)
		...
		plot = {'Date' : self.dates,
		'Actual_Price': self.close_price
		} #create a dictionary with dates and close values

		dfPlot = pd.DataFrame(plot, columns = ['Date', 'Actual_Price'])
		dfPlot.set_index("Date", inplace=True)
		figure = plt.Figure(figsize=(6, 6), dpi=105)
		figure_canvas = FigureCanvasTkAgg(figure, self.page)
		axes = figure.add_subplot()
	...
	def runNewTrade(self, data, numberOfRuns, startTime, currentAssetAmount, Currency, holdProfit, profit, HoldAmount, startingCurrency):
		...
		for i in range(0, numberOfRuns): #run the trading simulation for as many days as the predictions
			...
			if data['Buy'][i] == 1: #check the buying signals (if the RSI goes below 20)
				if(Currency!= 0 ): #if the bot has currency
					action = "Buying"
					Currency, currentAssetAmount = self.buyCoins(currentCoinPrice, Currency, currentAssetAmount)
					profit = (currentAssetAmount * currentCoinPrice) - startingCurrency
			...
			elif data['Sell'][i] == 2:#check the selling signals (if the RSI goes above 70)
				if(currentAssetAmount != 0): #if there are coins to sell
					action = "Selling"
					Currency, currentAssetAmount = self.sellCoins(currentCoinPrice, Currency, currentAssetAmount)
					profit = Currency - startingCurrency
				else:
					action ="Waiting" # waiting for a good point to buy back in
					profit = Currency - startingCurrency
