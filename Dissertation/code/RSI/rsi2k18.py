...
class RSI2018:
    ...
    def ComputeRSI(self, coin, data, RSI_20):
        ...
        data['Buy'] = np.where(data['RSI_20'] <= 30, 1,0)
        data['Sell'] = np.where(data['RSI_20'] >= 70, 2,0)
        ...
        SellLine = { 'Date' : data['time_period_start'],
                    '70' : [70 for index , row in data.iterrows() ]
                 }

        BuyLine = { 'Date' : data['time_period_start'],
                    '30' : [30 for index , row in data.iterrows()]
                 }

        Plot2 = pd.DataFrame(SellLine, columns = ['Date', '70'])
        Plot3 = pd.DataFrame(BuyLine, columns = ['Date', '30'])
        ...
        figure_canvas = FigureCanvasTkAgg(figure, self.page)
        axes = figure.add_subplot()
        axes.set_title('RSI data from 2020-2021 for: ' + coin)
        axes.set_xlabel('Date')
    ...
    def saveToFile(self, coin, data): #save the results of the prediction to an excel file
        workbook = xlsxwriter.Workbook('results/RSI/RSI2018-' + coin + '-Calculations.xlsx')
    ...
    def runTrading(self, coin, data, numberOfDays):
        ...
        workbook = xlsxwriter.Workbook('results/RSI/RSI2018-' + coin + '-TradingBot.xlsx')
        worksheet = workbook.add_worksheet()
        worksheet.set_column('A:A', 25)
        worksheet.set_column('B:B', 15)
        worksheet.set_column('C:C', 15)
